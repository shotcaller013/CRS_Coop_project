    ...
    <template v-else-if="d">
      ...
      <div class="stats-row">
        ...
        <div class="stat-card">
          ...
          <div class="stat-sub">
            <span :style="{ color: d.stats.collection_rate_delta >= 0 ? 'var(--green-600)' : 'var(--red-500)' }">
        ...
        <div class="stat-card">
          ...
          <div class="stat-val" :style="{ color: d.stats.overdue_count > 0 ? 'var(--red-500)' : 'var(--green-600)' }">
      ...
      <div class="row row-21">
        ...
        <div class="card" v-if="isManagerOrAbove">
          ...
          <div class="sc-balance">
            ...
            <div class="stat-val sm" style="color:var(--green-600)">
          ...
          <div class="sc-rows">
            <div class="sc-row">
              ...
              <span class="mono" style="color:var(--green-600)">+{{ peso(d.share_capital.month_credits) }}</span>
            </div>
            <div class="sc-row">
              <span>Debits this month</span>
              <span class="mono" style="color:var(--red-500)">−{{ peso(d.share_capital.month_debits) }}</span>
            </div>
            <div class="sc-row sc-row-total">
              <span>Net movement</span>
              <span class="mono" :style="{ color: d.share_capital.month_net >= 0 ? 'var(--green-600)' : 'var(--red-500)' }">
...
function rateColor(rate) {
  if (rate >= 90) return 'var(--green-600)'
  if (rate >= 75) return 'var(--yellow-600)'
  return 'var(--red-500)'
}
...
.stat-card   { background:var(--surface-ground); border-radius:var(--border-radius-md); padding:12px 14px; }
.stat-label  { font-size:11px; text-transform:uppercase; letter-spacing:.5px; color:var(--text-color-secondary); margin-bottom:4px; }
.stat-val    { font-size:24px; font-weight:500; }
.stat-val.sm { font-size:18px; }
.stat-sub    { font-size:11px; color:var(--text-color-secondary); margin-top:2px; }
...
/* Cards */
.card        { background:var(--surface-card); border:0.5px solid var(--surface-border); border-radius:var(--border-radius-lg); padding:14px 16px; }
.card-title  { font-size:11px; font-weight:500; text-transform:uppercase; letter-spacing:.5px; color:var(--text-color-secondary); margin-bottom:10px; }

/* Legend */
.legend      { display:flex; flex-wrap:wrap; gap:10px; margin-bottom:8px; font-size:12px; color:var(--text-color-secondary); }
...
.sc-row-total{ font-weight:500; border-top:0.5px solid var(--surface-border); padding-top:5px; margin-top:2px; }

/* Feed */
.feed-item   { display:flex; gap:8px; align-items:flex-start; padding:6px 0; border-bottom:0.5px solid var(--surface-border); font-size:12px; }
...
.feed-meta   { font-size:11px; color:var(--text-color-secondary); }
...
.overdue-table th { text-align:left; padding:5px 8px; font-size:10px; font-weight:500; text-transform:uppercase; letter-spacing:.4px; color:var(--text-color-secondary); border-bottom:0.5px solid var(--surface-border); }
.overdue-table td { padding:6px 8px; border-bottom:0.5px solid var(--surface-border); }
.overdue-table tr:last-child td { border-bottom:none; }
.overdue-row  { cursor:pointer; transition:background .1s; }
.overdue-row:hover td { background:var(--surface-hover); }
.overdue-badge { font-size:10px; font-weight:500; padding:2px 7px; border-radius:4px; }
.overdue-badge.danger { background:var(--red-50); color:var(--red-800); }
.overdue-badge.warn   { background:var(--yellow-50); color:var(--yellow-800); }
